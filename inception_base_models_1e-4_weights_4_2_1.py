...
def train_alone_model(base_model, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        outputs, features = base_model.extract_features(inputs.float())

        loss= criterion_CE(outputs, targets) + \
              ins_sparse_loss(features[0])  + ins_sparse_loss(features[1])   + ins_sparse_loss(features[2]) + \
              ins_sparse_loss(features[3])  + ins_sparse_loss(features[4])   + ins_sparse_loss(features[5])
...
if __name__ == '__main__':
    ...
    if args.output_directory is None:
        args.output_directory = f'/results_inception/base_models_1e-5_weights_4_2_1/'
    ...
    for dataset in args.datasets:        
        for seed in new_seeds:
            ...
            set_seeds(seed)
            # base_model = LITE(output_directory=None, length_TS=length_TS, n_classes=len(np.unique(ytrain)), n_filters=[[32, 32, 32], 32, 32])
            input_shape = xtrain.shape[1:]
            nb_classes = len(np.unique(ytrain))
            base_model = Classifier_INCEPTION(input_shape, nb_classes)
            
            first_model_wts = copy.deepcopy(base_model.state_dict())
            torch.save(first_model_wts, output_directory + f'first_model.pt')

            criterion_CE = nn.CrossEntropyLoss()
            ins_sparse_loss = InstanceFeatureSparseLoss(lambda_reg=1e-5)
            ...
            best_model = Classifier_INCEPTION(input_shape, nb_classes)
