...
class Inception_block(nn.Module):
  ...
  def __init__(self, in_channels, out_channels, stride=1, use_residual=True, use_bottleneck=True):
    ...
    if self.in_channels == 1:
        ...
        # print(f'self.out_channels.sum(): {self.out_channels.sum()}')
    else:
    ...
    if self.out_channels[0] > 0:
        self.branch1 = nn.Conv1d(in_channels=self.in_channels, out_channels=self.out_channels[0], kernel_size=40,  stride=1, padding='same', bias=False)
        
    if self.out_channels[1] > 0:
        self.branch2 = nn.Conv1d(in_channels=self.in_channels, out_channels=self.out_channels[1], kernel_size=20,  stride=1, padding='same', bias=False)
        
    if self.out_channels[2] > 0:
        self.branch3 = nn.Conv1d(in_channels=self.in_channels, out_channels=self.out_channels[2], kernel_size=10,  stride=1, padding='same', bias=False)
        
    if self.out_channels[3] > 0:
        self.branch4 = nn.Sequential(
            nn.MaxPool1d(kernel_size=3, stride=1, padding=1),
            nn.Conv1d(in_channels=in_channels, out_channels=self.out_channels[3], kernel_size=1, stride=1, padding='same', bias=False),
        )
...
class Classifier_INCEPTION(nn.Module):
    ...
    def __init__(self, input_shape, nb_classes, nb_filters=np.array([[32, 32, 32, 32],
                                                                     [32, 32, 32, 32],
                                                                     [32, 32, 32, 32],
                                                                     [32, 32, 32, 32],
                                                                     [32, 32, 32, 32],
                                                                     [32, 32, 32, 32]]),
                 depth=6, residual=True):
        ...
        for d in range(depth):
            self.inception.append(Inception_block(1 if d == 0 else nb_filters[d-1].sum(), nb_filters[d]))
            
            if self.residual and d % 3 == 2: 
                self.shortcut.append(Residual_block(1 if d == 2 else nb_filters[2].sum(), nb_filters[d].sum()))


        self.avgpool1 = nn.AdaptiveAvgPool1d(1)
        self.fc1 = nn.Linear(nb_filters[d].sum(), self.nb_classes)
    ...
    def forward(self, x):
        ...
        for d in range(self.depth):
            ...
            # print(f'Layer {d+1} output: {x.shape}')
            if self.residual and d % 3 == 2:
                # print('In the second shortcut')
                # print('Inpur res shape: ', input_res.shape)
                # print('\n\n')

                y = self.shortcut[d//3](input_res)
                # print(f'Layer {d+1} residual output: {y.shape}')      
                # print('0000000000')         
