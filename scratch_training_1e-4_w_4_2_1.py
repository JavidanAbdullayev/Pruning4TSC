...
from utils import load_data, preprocess_data, create_directory, plot_loss_and_acc_curves, InstanceFeatureSparseLoss, CovarianceLoss2
...
def get_args():
    ...
    parser.add_argument('--num_filters', help='output directory parent', type=int, default=32,)
    ...
    parser.add_argument('--output-directory', help='output directory parent', type=str, default=None)
...
def train_alone_model(base_model, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        outputs, features = base_model.forward_sparsity(inputs.float())
...
def reinitialize_model_weights(model, seed):
    
    torch.manual_seed(seed)
    
    for name, module in model.named_modules():
        if hasattr(module, 'reset_parameters') and 'hybrid_block' not in name:
            module.reset_parameters()
            print('YESSSSSSSSSSSSSSSSSSSSS')
            print('name: ', name)
    return model

new_seeds = [9409, 3804, 3952, 2561, 5296]
# new_seeds = [9409]

if __name__ == '__main__':
    args = get_args()
    if args.output_directory is None:
        args.output_directory = f'/results/scratch_training_1e-4_weights_4_2_1_epochs_1500/'
    ...
    for dataset in args.datasets:        
        for seed in new_seeds:
            ...
            output_directory = output_directory + f'ins_sparse_seed_{seed}/' + dataset + '/'
            create_directory(output_directory)

            set_seeds(seed)
            pruned_model = torch.load(os.getcwd() + f'/results/base_models_1e-4_weights_4_2_1/ins_sparse_seed_{seed}/{dataset}/pruned_model_1e-4_w_4_2_1.pth',)
            pruned_model = reinitialize_model_weights(pruned_model, seed=seed)
            
            first_model_wts = copy.deepcopy(pruned_model.state_dict())
            ...
            print('Pruned model performance before training: ')
            val_loss, val_acc = test(pruned_model)
            ...
            for epoch in range(epochs):
                train_loss, train_acc = train_alone_model(pruned_model, epoch)                
                ...
                if min_train_loss >= train_loss:
                    ...
                    torch.save(pruned_model, output_directory +  '/best_model.pth')
            ...
            last_model_wts = copy.deepcopy(pruned_model.state_dict())
            torch.save(pruned_model, output_directory +  '/last_model.pth')
            ...
            # best_model = LITE(output_directory=None, length_TS=length_TS, n_classes=len(np.unique(ytrain)), n_filters=[[32, 32, 32], 32, 32])
            best_model = torch.load(output_directory + 'best_model.pth')
